...
logistic_predictions <- predict(logistic_fit, new_data = cces) %>%
  bind_cols(cces) %>% 
  accuracy(truth = trump, estimate = .pred_class)


tree_predictions <- predict(vote_tree, new_data = cces) %>%
  bind_cols(cces) %>% 
  accuracy(truth = trump, estimate = .pred_class)

forest_predictions <- predict(vote_forest, new_data = cces) %>%
  bind_cols(cces) %>% 
  accuracy(truth = trump, estimate = .pred_class)


bind_rows(logistic_predictions, tree_predictions, forest_predictions)
